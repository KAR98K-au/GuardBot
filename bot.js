...
http
  .createServer(function(req, res) {
    res.write("Global Guard Bot EN - TR!");
    res.end();
  })
  .listen(8080);
//
...
client.on("message", async message => {
  ...
  if (message.content === "g-") {
    ...
    if (dil == "TR_tr") {
      message.channel.send(
        "Galiba komut ismini unuttun, `g-yardım`` yazarak gerekli komutlara erişebilirsiniz."
      );
    } else {
      message.channel.send(
        "I think you forgot the command name, you can access the necessary commands by typing `g-help`"
      );
    }
  } else {
    return;
